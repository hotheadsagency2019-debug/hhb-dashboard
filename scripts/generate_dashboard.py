...
def direct_request(token: str, login: str, service: str, method: str, params: dict) -> dict:
    """Отправляет запрос к Яндекс Директ API."""
...
def get_campaigns(token: str, login: str) -> list:
    """Получает активные кампании аккаунта."""
...
def get_campaign_stats(token: str, login: str, date_from: str, date_to: str) -> list:
    """Получает статистику по кампаниям за период."""
    resp = direct_request(token, login, "reports", "get", {
        "params": {
            "SelectionCriteria": {
                "DateFrom": date_from,
                "DateTo": date_to,
            },
            "FieldNames": ["Date", "CampaignId", "CampaignName", "CampaignType",
                           "Impressions", "Clicks", "Cost"],
            "ReportName": f"hhb_report_{date_from}_{date_to}",
    ...
    # Reports API возвращает TSV — парсим
    rows = []
...
def get_direct_stats_v2(token: str, login: str, date_from: str, date_to: str) -> dict:
    """
    Получает агрегированную статистику через Campaigns.get со Statistics.
    Возвращает totals + daily (если нет детальных дат — агрегат за период).
    """
    ...
    for c in campaigns:
        ...
        cost = float(stats.get("Cost", 0) or 0)  # уже в рублях
...
def metrica_request(token: str, counter_id: str, params: dict) -> dict:
    """Отправляет запрос к Яндекс Метрике."""
...
def get_metrica_daily(token: str, counter_id: str, date_from: str, date_to: str) -> dict:
    """Получает дневную статистику из Метрики."""
...
def get_metrica_goals(token: str, counter_id: str, date_from: str, date_to: str,
                      goal_ids: list) -> dict:
    """Получает данные по целям из Метрики."""
    if not goal_ids:
    ...
    for i, gid in enumerate(goal_ids):
        ...
        reaches = totals_raw[i * 2 + 1] if len(totals_raw) > i * 2 + 1 else 0
        goals.append({"goal_id": gid, "reaches": int(reaches), "conv_rate": round(conv_rate, 2)})

    return {"available": True, "goals": len(goals), "data": goals}
...
def build_account_data(token: str, account: dict, date_from: str, date_to: str,
    ...
    """Собирает все данные по одному аккаунту."""
    ...
    direct_stats = get_direct_stats_v2(token, login, date_from, date_to)

    metrica_current = {"daily": [], "totals": {}}
    metrica_prev = {"daily": [], "totals": {}}
    goals_data = {"available": False}
    ...
    if counter_id:
        ...
        if goal_ids:
            ...
            # Вписываем leads в daily из целей (суммируем все цели)
            total_leads = sum(g["reaches"] for g in goals_data.get("data", []))
            if metrica_current["daily"]:
                per_day = total_leads // len(metrica_current["daily"])
                for row in metrica_current["daily"]:
                    row["leads"] = per_day
            metrica_current["totals"]["leads"] = total_leads

    # Вычисляем сравнение периодов для Директа
    prev_direct = get_direct_stats_v2(token, login, prev_date_from, prev_date_to)
    ...
    return {
        ...
        "coverage": {
            "direct_current_daily": {"count": 1},
            "metrica_current_daily": {"count": len(metrica_current["daily"])},
            "metrica_goals": goals_data,
        },
        "direct": {
            "current": {
                "daily": [],  # агрегат без разбивки по дням (используем totals)
                "totals": ct,
            },
            "prev": {
                "daily": [],
            ...
            "campaign_summaries": direct_stats["campaign_summaries"],
            "change": change,
        },
        "metrica": {
...
def generate_html(data: dict, template_path: Path) -> str:
    """Вставляет JSON данные в HTML-шаблон."""
    template = template_path.read_text(encoding="utf-8")
    json_data = json.dumps(data, ensure_ascii=False, indent=2)
    # Заменяем плейсхолдер в шаблоне
    html = template.replace("__DASHBOARD_DATA_PLACEHOLDER__", json_data)
    return html
...
def main():
    ...
    # Даты
    today = datetime.date.today() - datetime.timedelta(days=1)  # вчера = полные данные
    ...
    print(f"Период: {date_from} — {date_to} (сравнение: {prev_date_from} — {prev_date_to})")

    # Загружаем аккаунты
    accounts_path = Path(args.accounts)
    if not accounts_path.exists():
        print(f"ERROR: Файл аккаунтов не найден: {accounts_path}", file=sys.stderr)
        sys.exit(1)
    ...
    for acc in accounts:
        ...
        try:
            ...
            accounts_data[acc["id"]] = {"error": str(e), "meta": {"project_name": acc["name"]}}
    ...
    if not template_path.exists():
        print(f"ERROR: Шаблон не найден: {template_path}", file=sys.stderr)
        sys.exit(1)
